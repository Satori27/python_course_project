...
def send_user_stat_to_kafka(message: dict, topic: str = 'user-stats'):
    """
    ...
    try:
        ...
        logger.info(f"Message {message} sent to {record_metadata.topic} partition {record_metadata.partition} offset {record_metadata.offset}")
    except Exception as e:
        logger.error(f"Failed to send message: {e}")
        raise e